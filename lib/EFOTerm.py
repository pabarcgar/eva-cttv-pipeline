...
def executeQuery(sparqlep, user, password, query):
    fd = os.popen("""curl --post301 -L --data-urlencode "query="""+query+"""" -u """+user+":"+password+""" -H "Accept: text/tab-separated-values" """+sparqlep)
...
class EFOTerm():
    ...
    sparqlep="http://beta.targetvalidation.org/proxy/sparql"
    user="cttvdataprovider"
    password="ABA9vvNUdd"
